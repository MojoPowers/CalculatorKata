package com.kata.Bobrov.lines;

import com.kata.Bobrov.converter.Converter;
import com.kata.Bobrov.exception.ArithmeticOperationException;
...
public class LinePreparing {
    ...
    private static String[] findMathSymbol(String source) {
        ...
        if (mathString.contains("*")) {
            ...
            throw new ArithmeticOperationException("т.к. строка не является математической операцией");
        }
    }
    ...
    private static String[] checkLine(String[] source) {
        if (Character.isDigit((source[0]).charAt(0)) && Character.isDigit((source[2]).charAt(0))) {
            ...
            throw new ArithmeticOperationException("т.к. используются одновременно разные системы счисления");
        }
    }
    ...
    private static String[] checkOperation(String[] source) {
        if (source[0].contains("*") || source[1].contains("*") ||
            ...
            throw new ArithmeticOperationException("т.к. формат математической операции не удовлетворяет заданию" +
